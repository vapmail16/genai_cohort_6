...
**Commands** (from `capstone_project/` — same layout in this bundle and in the main cohort repo):

```bash
cd capstone_project
# Demo SQLite rows for DB RAG (tickets + chat history)
python3 -m backend.seed_demo_data --reset
# Markdown → Qdrant collection it_support_kb
...
| 2026-06-20 | `MCPArchitecture.html` and `ITSupportArchitecture.html` opened as blank pages in the browser. | `unpkg.com/@babel/standalone` now resolves to Babel 8, which no longer auto-compiles `text/babel` scripts without explicit presets/targets. Pin `@babel/standalone@7.26.9` in both HTML files and serve via a local HTTP server (e.g. `cd mcp && python3 -m http.server 8765`) rather than `file://`. |
| 2026-06-20 | RAG/agent demos had no documented one-shot seed path for SQLite + Qdrant. | Run `python -m backend.seed_demo_data --reset` then `python -m backend.rag.ingest --reset` from `capstone_project/` (see docs README cheat sheet). |
